document.addEventListener('DOMContentLoaded', () => {
  ...
  function handleBuildPhase() {
      ...
      selectedGrid.addEventListener('click', function selectCell(event) {
          if (event.target.style.backgroundColor !== 'gray' && selectedCells.length < 6 && !event.target.classList.contains('disabled')) {
              ...
              selectedCells.push(parseInt(event.target.id.split('-')[1]));
  ...
  randomButton.addEventListener('click', () => {
      if (randomButton.classList.contains('red') || randomButton.classList.contains('blue')) {
          ...
          interval = setInterval(() => {
              ...
              // Check if the selected cell has a letter
              if (selectedCells1.includes(availableCells[randomIndex]) || selectedCells2.includes(availableCells[randomIndex])) {
                  ...
                  if (redScore === 0 || blueScore === 0) {
                      clearInterval(interval);
                      clearInterval(countdownInterval);
                      statusBox.textContent = `${currentPlayer === 'red' ? 'Blue' : 'Red'} Player Wins!`;
  ...
  restartButton.addEventListener('click', () => {
      ...
      document.querySelectorAll('.grid div').forEach(div => {
          ...
          div.textContent = ''; // Clear the text content
      });
      redScore = 6;
      blueScore = 6;
      redScoreElement.textContent = redScore;
